...
function Header({ theme, setTheme}) {
  ...
  return (
    <header id="home" className=" h-[50vh] lg:h-screen bg-gradient-to-t from-indigo-200  dark:from-slate-900 dark:to-slate-600 relative overflow-hidden">
    {/* Navbar */}
    <nav className="w-full fixed top-0 bg-white z-10 dark:bg-slate-900">
        <div className="container mx-auto py-5 px-5 sm:px-0 lg:px-5 flex items-center justify-between">
            ...
            {/* Dark mode */}
            <img className="hidden md:block w-10 cursor-pointer bg-slate-600 p-2 rounded-full" src={moon} alt="moon" onClick={handleThemeSwitch}/>
            {/* Hamburger */}
    ...
    {/* text */}
    <div className="hidden lg:flex flex-col gap-5 rounded-md shadow-lg dark:bg-slate-900 dark:shadow-slate-800 absolute top-40 bottom-0 m-auto right-10 bg-white p-6 h-fit w-1/4">
        <h1 className="text-4xl font-bold text-indigo-900 dark:text-white">Hi, Im Daniel</h1>
        <p className="text-gray-400">
            Will code for food. 
        </p>
        <a className="bg-yellow-400 text-white text-xl px-3 py-2 rounded-md font-semibold w-fit" href="#contact">Hire Me</a>
    </div>
</header>
  )
}
